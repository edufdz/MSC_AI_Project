...
export function bootstrapFakeEnvironment(): FakeSupabaseClient {
    ...
    // would open a real network path to Samsung/Pulpoo infra is neutralized.
    ...
    if (!process.env.ANTHROPIC_API_KEY) {
        throw new Error(
            "ANTHROPIC_API_KEY is required (the agent's LLM calls are real; only the data layer is fake). Add it to samsung-live-agent/.env",
        );
    }
    ...
    setAppId("fake-samsung-simulation");

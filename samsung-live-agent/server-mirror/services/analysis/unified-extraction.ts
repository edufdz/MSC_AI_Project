...
export const UnifiedInteractionSchema = z.object({
  ...
  primary_device_model: z
    ...
    .optional()
    .describe(
      "Samsung device model mentioned (e.g., Galaxy S24 Ultra, Galaxy A52)"
    ),
  warranty_status: z
  ...
  claimed_model_name: z
    ...
    .optional()
    .describe(
      "Specific Samsung model the customer mentioned on this call (e.g., 'Galaxy S25', 'Note 20 Ultra'). May differ from primary_device_model when the customer is asking about a device other than their main one. Omit if not mentioned."
    ),
  reported_symptom: z
    .string()
    .optional()
...
const EXTRACTION_PROMPT = `Analiza la siguiente transcripción de interacción con Samsung (llamada o WhatsApp) y extrae:
...
6. primary_device_model: Modelo de dispositivo Samsung mencionado (ej: "Galaxy S24 Ultra", "Galaxy A52"). Omitir si no se menciona.
...
14. claimed_model_name: El modelo Samsung específico que el cliente mencionó EN ESTA llamada (ej: "Galaxy S25", "Note 20 Ultra"). Puede diferir de primary_device_model. Omitir si no se mencionó.

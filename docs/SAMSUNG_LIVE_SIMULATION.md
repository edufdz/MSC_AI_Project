# Samsung Live Simulation — What Was Built and Where Everything Lives
...
Samsung agent simulation**, the **Phase B suite generated for it**, the
...
The real Samsung/Pulpoo WhatsApp agent was copied **verbatim** out of
`pulpoo-final` into a new self-contained project, `samsung-live-agent/`,
where it runs against a **fake in-memory database** seeded with one fully
specified fake customer — no connection whatsoever to Samsung systems,
Supabase, GSPN, or Meta. Phase B then generated a 40-test Spanish suite from
the Samsung agent map, and Phase C executed that suite against the living
...
## 2. The living agent — `samsung-live-agent/`
...
| Path | What it is |
|---|---|
| `samsung-live-agent/server.ts` | HTTP harness: `POST /chat` (Phase C contract), `GET /db` (inspect fake DB), `POST /reset` (fresh DB between runs), `GET /health` |
| `samsung-live-agent/index.ts` | Interactive CLI chat (`bun run cli`) |
| `samsung-live-agent/bootstrap.ts` | Plays pulpoo's "App" role: injects the fake Supabase client + app id, pins env so nothing reaches real infra |
| `samsung-live-agent/context-builder.ts` | Builds the `AssembledContext` the agent expects (the gateway's job in production), sourced from the fake DB |
| `samsung-live-agent/fake-db/fake-supabase.ts` | In-memory Supabase-compatible query builder (filters, `.or()`, JSON paths, upserts, 3 Postgres RPCs). Records every mutation in a `mutationLog` |
| `samsung-live-agent/fake-db/seed.ts` | All tables + THE fake customer (below) |
| `samsung-live-agent/server-mirror/` | Mirrors pulpoo's `server/` layout; tsconfig maps `@/*` here. Contains the **entire WhatsApp agent verbatim** (`services/agents/whatsapp/` — prompts, graph, 12 nodes, event detectors, order-lookup tool, style guide) plus ~46 support modules copied verbatim |
| `samsung-live-agent/README.md` | Full run instructions, fake-vs-real boundary table, verified behaviors |
...
## 3. Phase B — the generated Samsung suite

### 3.1 Where it is

**`debugger-platforn/generated_samsung/`** (committed; the platform's
...
1. `samsung-live-agent` exposes `POST /chat` — the exact
...
2. `debugger-platforn/agent_endpoints.json` resolves the Samsung map to
   `http://localhost:3098` automatically (and
   `samsung_whatsapp_map_live.json` also carries `api_endpoint` directly).
3. Run Phase C **without `--mock`**:

```bash
# terminal 1
cd samsung-live-agent && bun run api
# terminal 2
cd debugger-platforn && source venv/bin/activate
python execute_tests.py generated_samsung/test_suite.json \
    samsung_whatsapp_map_live.json --count 40 --workers 2 -o results
```

> **Always use `samsung_whatsapp_map_live.json` for execution.** The
> pristine research map (`samsung_whatsapp_map.json`) has zero terminal
...
|---|---|---|---|
| **v1** | `samsung_whatsapp_map.json` (research map) | **0/10, all vacuous** ("max turns" / "persona gave up"), 0% tool coverage, avg 82.6s/test | `debugger-platforn/results_samsung_live/` |
| **v2** | `samsung_whatsapp_map_live.json` | **9/10 pass**: 7× `order_status_provided`, 2× `escalated_to_human`, 1 genuine `user_abandoned` failure; avg 19.1s/test | `debugger-platforn/results_samsung_live_v2/` |
...
- `samsung-live-agent/server.ts` now mirrors each `tool_output` into
  `result: {status: "ok"|"error"}` and emits a synthetic
  `escalate_to_human` tool call when the agent escalates.
- `samsung_whatsapp_map_live.json` = copy of the research map + runtime
...
| I want… | Look in |
|---|---|
| The living agent + how to run it | `samsung-live-agent/README.md` |
| The fake customer's data | `samsung-live-agent/fake-db/seed.ts` |
| The verbatim prompts | `samsung-live-agent/server-mirror/services/agents/whatsapp/prompts/` |
| Phase B suite + quality report | `debugger-platforn/generated_samsung/` |
| Execution map (use this one) | `debugger-platforn/samsung_whatsapp_map_live.json` |
| Research map (keep pristine) | `debugger-platforn/samsung_whatsapp_map.json` |
| Endpoint wiring | `debugger-platforn/agent_endpoints.json` |
| Oracle-gap evidence run (0/10) | `debugger-platforn/results_samsung_live/` |
| Working run (9/10) | `debugger-platforn/results_samsung_live_v2/` |
